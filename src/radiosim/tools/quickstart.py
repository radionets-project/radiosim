...
def main(
    ...
    """Quickstart CLI tool for radiosim. Creates
    ...
    a file called 'default_simulation.toml'
    inside that directory.
    """
    log = setup_logger(tracebacks_suppress=[click])

    msg = f"This is the radiosim [blue]v{__version__}[/] quickstart tool"
    ...
    default_config_path = Path(root + "/share/configs/default_simulation.toml")

    with open(default_config_path) as f:
        default_config = toml.load(f)

    log.info("Loading default radiosim configuration:")
    log.info(pretty_repr(default_config))

    if config_path.is_dir():
        config_path /= "default_simulation.toml"
